...
const BROKER_RE = /(?:makler|vositachi|ри[еэ]лтор|агентств|broker|realtor)/iu;
...
const FAMILY_RE = /(?:семь\p{L}*|family|oilaga|oila\s+uchun|оилага)/iu;
const ROOM_SHARE_RE = /(?:sherik(?:ka|lik|likga)?|шерик(?:ка|лик)?|roommate|flatmate|подселени|койко[-\s]?мест|место\s+в\s+(?:комнат|квартир))/iu;
