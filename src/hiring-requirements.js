...
export function isNoSponsorshipRequirement(value) {
  const text = String(value || '');
  return /(?:\bno\s+(?:visa\s+|immigration\s+|employment\s+)?sponsorship\b|\b(?:will\s+not|cannot|can't|unable\s+to|not\s+able\s+to)\s+sponsor\b|\bwithout\s+(?:the\s+need\s+for\s+)?(?:current\s+or\s+future\s+)?(?:employer\s+|visa\s+)?sponsorship\b|\bmust\s+(?:be\s+)?(?:legally\s+)?authoriz\w+\s+to\s+work[^.!?]{0,100}\bwithout\s+(?:current\s+or\s+future\s+)?sponsorship\b|\bsponsorship\s+(?:is\s+)?not\s+(?:available|provided|offered)\b|\bmay\s+not\s+be\s+able\s+to\b[^.!?]{0,450}\b(?:sponsor|support|provide)\b[^.!?]{0,180}\bsponsorship\b|\b(?:will|can|may)\s+not\b[^.!?]{0,220}\b(?:sponsor|support|provide)\b[^.!?]{0,160}\bsponsorship\b|\bnot\s+(?:currently\s+)?(?:able\s+to\s+)?(?:sponsor|support|provide)\b[^.!?]{0,160}\bsponsorship\b)/i.test(text);
}
