...
test('temporal API ignores an invalid explicit reference date and records its valid fallback', () => {
  const parsed = parseTemporal('Available from January 12', {
    domain: 'real-estate', referenceDate: 'not a date', publishedAt: '2026-12-20T12:00:00Z',
  });
  assert.deepEqual(parsed.data.availabilityDate, { year: 2027, month: 1, day: 12 });
  assert.ok(parsed.debug.candidates.find((item) => item.entityType === 'availabilityDate')
    .evidence.some((item) => item.type === 'inferred-year' && item.reference === 'publishedAt'));
});
